<template>
  <div class="junpintan-container" @scroll="monitorScrollEvent" ref="junpintan-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="junpintan-list" v-if="!pageLoading">
        <div class="swiper-container bg-white junpintanSwiperList-container" style="height: 240px" v-if="junpintanSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide junpintanSwiperList-slide" v-for="(news, index) in junpintanSwiperList" :key="index" style="width: 100%">
        ...
        <div class="junpintanNewsList" v-if="junpintanNewsList.length">
          <li v-for="news in junpintanNewsList" :key="news.id + Math.random().toString()">
            <!-- doc -->
...
import { TabHomeJunpintanModule } from '@/store/modules/tab_home_junpintan';

@Component({
  name: 'tabs_home_item_junpintan',
})
export default class extends Vue {
  ...
  private junpintanNewsList: any[] = [];
  private junpintanSwiperList = [];
  // 下拉刷新，上拉加载的数据
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.junpintanNewsList.length ? `已为您推荐 ${this.junpintanNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['junpintan-container'].scrollHeight;
    const scrollTop = this.$refs['junpintan-container'].scrollTop;
  ...
  private async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'JUNPT', ch: 'jpt', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeJunpintanModule.getRecomlistForJunpintan({ params, formData });
      const arr = [];
      ...
      this.junpintanNewsList = arr;
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.junpintanSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.junpintanSwiperList = result[1].item;
      } else {
        this.junpintanSwiperList = [];
      }
  ...
  private async _upCallback() {
    ...
    try {
      ...
      this.junpintanNewsList = this.junpintanNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
