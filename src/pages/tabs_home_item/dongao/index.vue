<template>
  <div class="dongao-container" @scroll="monitorScrollEvent" ref="dongao-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="dongao-list" v-if="!pageLoading">
        <div class="swiper-container bg-white dongaoSwiperList-container" style="height: 240px" v-if="dongaoSwiperList.length">
        ...
        <div class="dongaoNewsList" v-if="dongaoNewsList.length">
          <li v-for="news in dongaoNewsList" :key="news.id + Math.random().toString()">
            <!-- doc -->
...
import { TabHomeDongaoModule } from '@/store/modules/tab_home_dongao';
@Component({
  name: 'tabs_home_item_dongao',
})
export default class extends Vue {
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.dongaoNewsList.length ? `已为您推荐 ${this.dongaoNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['dongao-container'].scrollHeight;
    const scrollTop = this.$refs['dongao-container'].scrollTop;
  ...
  private async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'WO22', ch: 'winoly', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeDongaoModule.getDongaoNewsList({ params, formData });
      const arr = [];
      ...
      this.dongaoNewsList = arr;
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.dongaoSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.dongaoSwiperList = result[1].item;
      } else {
        this.dongaoSwiperList = [];
      }
  ...
  private async _upCallback() {
    ...
    try {
      ...
      this.dongaoNewsList = this.dongaoNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
