<template>
  <div class="junshi-container" @scroll="monitorScrollEvent" ref="junshi-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="junshi-list" v-if="!pageLoading">
        <div class="swiper-container bg-white junshiSwiperList-container" style="height: 240px" v-if="junshiSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide junshiSwiperList-slide" v-for="(news, index) in junshiSwiperList" :key="index" style="width: 100%">
        ...
        <div class="junshiGridList" v-if="junshiGridList.length">
          <li v-for="(item, index) in junshiGridList" :key="index" @click="handlerClickGridItem(item, index)">
            <img :src="item.thumbnail" alt="" />
            <p class="title">{{ item.title }}</p>
          </li>
        </div>
        <div class="junshiNewsList" v-if="junshiNewsList.length">
          <li v-for="news in junshiNewsList" :key="news.id + Math.random().toString()">
            <!-- doc -->
...
import { TabHomeJunshiModule } from '@/store/modules/tab_home_junshi';

@Component({
  name: 'tabs_home_item_junshi',
})
export default class extends Vue {
  ...
  private junshiNewsList: any[] = [];
  private junshiSwiperList = [];
  private junshiGridList = [];
  // 下拉刷新，上拉加载的数据
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.junshiNewsList.length ? `已为您推荐 ${this.junshiNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['junshi-container'].scrollHeight;
    const scrollTop = this.$refs['junshi-container'].scrollTop;
  ...
  private handlerClickGridItem(item: any, index: number) {
    ...
    if (index === 0) {
      this.$router.push('/news_topic?topicid=' + item.documentId);
    }
  }
  /*http*/
  private async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'JS83', ch: 'mil', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeJunshiModule.getRecomlistForJunshi({ params, formData });
      const arr = [];
      ...
      this.junshiNewsList = arr;
      if (result.length === 3) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.junshiSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.junshiSwiperList = result[1].item;
        this.junshiGridList = result[2].item;
      } else {
        this.junshiGridList = result[1].item;
        this.junshiSwiperList = [];
      }
  ...
  private async _upCallback() {
    ...
    try {
      ...
      this.junshiNewsList = this.junshiNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
