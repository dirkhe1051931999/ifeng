<template>
  <div class="tab_keji_child_huaixiaoxi" ref="tab_keji_child_huaixiaoxi" @scroll="monitorScrollEvent">
    <div class="back">
    ...
    <ul class="list" v-if="huaixiaoxiList.length">
      <li v-for="(news, index) in huaixiaoxiList" :key="index">
        <!-- doc -->
...
@Component({
  name: 'tab_keji_child_huaixiaoxi',
})
export default class extends Vue {
  ...
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['tab_keji_child_huaixiaoxi'].scrollHeight;
    const scrollTop = this.$refs['tab_keji_child_huaixiaoxi'].scrollTop;
  ...
  private async getList() {
    try {
      ...
      const result = await TabHomeKejiModule.getKejiNewsHuaixiaoxiList({ params, formData });
      this.huaixiaoxiList = result[0].item;
  ...
  private async getMoreList() {
    try {
      ...
      this.huaixiaoxiList = this.huaixiaoxiList.concat(result[0].item);
      this.$nextTick(() => {
