<template>
  <div class="tab_keji_child_xinshijie" ref="tab_keji_child_xinshijie" @scroll="monitorScrollEvent">
    <div class="back">
    ...
    <ul class="list" v-if="xinshijieList.length">
      <li v-for="(news, index) in xinshijieList" :key="index">
        <!-- doc -->
...
export default class extends Vue {
  ...
  private async getList() {
    try {
      ...
      const result = await TabHomeKejiModule.getKejiNewsXinshijieList({ params, formData });
      this.xinshijieList = result[0].item;
  ...
  private async getMoreList() {
    try {
      ...
      this.xinshijieList = this.xinshijieList.concat(result[0].item);
      this.$nextTick(() => {
