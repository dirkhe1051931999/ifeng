<template>
  <div class="tab_keji_child_yuanchuang" ref="tab_keji_child_yuanchuang" @scroll="monitorScrollEvent">
    <div class="back">
    ...
    <ul class="list" v-if="yuanchuangList.length">
      <li v-for="(news, index) in yuanchuangList" :key="index">
        <!-- doc -->
...
@Component({
  name: 'tab_keji_child_yuanchuang',
})
export default class extends Vue {
  ...
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['tab_keji_child_yuanchuang'].scrollHeight;
    const scrollTop = this.$refs['tab_keji_child_yuanchuang'].scrollTop;
  ...
  private async getList() {
    try {
      ...
      const result = await TabHomeKejiModule.getKejiNewsYuanchuangList({ params, formData });
      this.yuanchuangList = result[0].item;
  ...
  private async getMoreList() {
    try {
      ...
      this.yuanchuangList = this.yuanchuangList.concat(result[0].item);
      this.$nextTick(() => {
