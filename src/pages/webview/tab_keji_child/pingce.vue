<template>
  <div class="tab_keji_child_pingce" ref="tab_keji_child_pingce" @scroll="monitorScrollEvent">
    <div class="back">
    ...
    <ul class="list" v-if="pingceList.length">
      <li v-for="(news, index) in pingceList" :key="index">
        <!-- doc -->
...
export default class extends Vue {
  ...
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['tab_keji_child_pingce'].scrollHeight;
    const scrollTop = this.$refs['tab_keji_child_pingce'].scrollTop;
  ...
  private async getList() {
    try {
      ...
      const result = await TabHomeKejiModule.getKejiNewsPingceList({ params, formData });
      this.pingceList = result[0].item;
  ...
  private async getMoreList() {
    try {
      ...
      this.pingceList = this.pingceList.concat(result[0].item);
      this.$nextTick(() => {
