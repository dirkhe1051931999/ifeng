<template>
  <div class="tab_wenchuang_child_jingxuan" ref="tab_wenchuang_child_jingxuan" @scroll="monitorScrollEvent">
    <div class="back">
    ...
    <ul class="list" v-if="jingxuanList.length">
      <li v-for="(news, index) in jingxuanList" :key="index">
        <!-- doc -->
...
@Component({
  name: 'tab_wenchuang_child_jingxuan',
})
export default class extends Vue {
  ...
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['tab_wenchuang_child_jingxuan'].scrollHeight;
    const scrollTop = this.$refs['tab_wenchuang_child_jingxuan'].scrollTop;
  ...
  private async getList() {
    try {
      ...
      const result = await TabHomeWenchuangModule.getWenchuangNewsJingxuanList({ params, formData });
      this.jingxuanList = result[0].item;
  ...
  private async getMoreList() {
    try {
      ...
      this.jingxuanList = this.jingxuanList.concat(result[0].item);
      this.$nextTick(() => {
