<template>
  <div class="keji-container" @scroll="monitorScrollEvent" ref="keji-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="keji-list" v-if="!pageLoading">
        <div class="swiper-container bg-white kejiSwiperList-container" style="height: 240px" v-if="kejiSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide kejiSwiperList-slide" v-for="(news, index) in kejiSwiperList" :key="index" style="width: 100%">
        ...
        <div class="kejiGridList" v-if="kejiGridList.length">
          <li v-for="(item, index) in kejiGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <div class="kejiNewsList" v-if="kejiNewsList.length">
          <li v-for="news in kejiNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
export default class extends Vue {
  ...
  public kejiNewsList: any[] = [];
  public kejiSwiperList: any[] = [];
  public kejiGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.kejiNewsList.length ? `已为您推荐 ${this.kejiNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['keji-container'].scrollHeight;
    const scrollTop = this.$refs['keji-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    switch (item.title) {
      case '科技热榜':
        this.$router.push('/tab_keji_child/rebang');
        break;
      case '新视界':
        this.$router.push('/tab_keji_child/xinshijie');
        break;
      case '凰家评测':
        this.$router.push('/tab_keji_child/pingce');
        break;
      case '坏消息':
        this.$router.push('/tab_keji_child/huaixiaoxi');
        break;
      case '原创':
        this.$router.push('/tab_keji_child/yuanchuang');
        break;
      case '深度':
        this.$router.push('/tab_keji_child/shendu');
        break;
      default:
        break;
    }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'KJ123', ch: 'tech', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeKejiModule.getKejiNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      this.kejiNewsList = arr;
      for (let item of result[2].item) {
        if (item.type !== 'gregnewslist') {
          arr2.push(item);
        }
      }
      this.kejiGridList = arr2;
      if (result.length === 3) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.kejiSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.kejiSwiperList = result[1].item;
      } else {
        this.kejiSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.kejiNewsList = this.kejiNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
