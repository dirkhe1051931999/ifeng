<template>
  <div class="qiche-container" @scroll="monitorScrollEvent" ref="qiche-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="qiche-list" v-if="!pageLoading">
        <div class="swiper-container bg-white qicheSwiperList-container" style="height: 240px" v-if="qicheSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide qicheSwiperList-slide" v-for="(news, index) in qicheSwiperList" :key="index" style="width: 100%">
        ...
        <div class="qicheGridList" v-if="qicheGridList.length">
          <li v-for="(item, index) in qicheGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <div class="qicheNewsList" v-if="qicheNewsList.length">
          <li v-for="news in qicheNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeQicheModule } from '@/store/modules/home_tab/qiche';
import { TabHomeChengshiModule } from '@/store/modules/home_tab/chengshi';
@Component({
  name: 'home_tab_item_qiche',
})
export default class extends Vue {
  ...
  public qicheNewsList: any[] = [];
  public qicheSwiperList: any[] = [];
  public qicheGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.qicheNewsList.length ? `已为您推荐 ${this.qicheNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['qiche-container'].scrollHeight;
    const scrollTop = this.$refs['qiche-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_qiche_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_qiche_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_qiche_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_qiche_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_qiche_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_qiche_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'QC45', ch: 'auto', action: 'default', pullTotal: '1', dailyOpenNum: '1', choicename: this.currentRegion };
      }
      const result = await TabHomeQicheModule.getQicheNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      this.qicheNewsList = arr;
      for (let item of result[2].item) {
        if (item.type !== 'gregnewslist') {
          arr2.push(item);
        }
      }
      this.qicheGridList = arr2;
      if (result.length === 3) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.qicheSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.qicheSwiperList = result[1].item;
      } else {
        this.qicheSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.qicheNewsList = this.qicheNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
