<template>
  <div class="jiankang-container" @scroll="monitorScrollEvent" ref="jiankang-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="jiankang-list" v-if="!pageLoading">
        <div class="swiper-container bg-white jiankangSwiperList-container" style="height: 240px" v-if="jiankangSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide jiankangSwiperList-slide" v-for="(news, index) in jiankangSwiperList" :key="index" style="width: 100%">
        ...
        <div class="jiankangGridList" v-if="jiankangGridList.length">
          <li v-for="(item, index) in jiankangGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <van-notice-bar left-icon="volume-o" :scrollable="false" color="#1989fa" background="#ecf9ff" v-if="fastmessagescrollList.length">
          <van-swipe vertical class="yaowen-notice-swipe" :healthplay="3000" :show-indicators="false">
            <van-swipe-item v-for="(item, index) in fastmessagescrollList" :key="index" class="text-dot-1">{{ item.title }}</van-swipe-item>
          </van-swipe>
        </van-notice-bar>
        <div class="jiankangNewsList" v-if="jiankangNewsList.length">
          <li v-for="news in jiankangNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeJiankangModule } from '@/store/modules/home_tab/jiankang';
import { TabHomeChengshiModule } from '@/store/modules/home_tab/chengshi';
@Component({
  name: 'home_tab_item_jiankang',
})
export default class extends Vue {
  ...
  public jiankangNewsList: any[] = [];
  public jiankangSwiperList: any[] = [];
  public jiankangGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.jiankangNewsList.length ? `已为您推荐 ${this.jiankangNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['jiankang-container'].scrollHeight;
    const scrollTop = this.$refs['jiankang-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_jiankang_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_jiankang_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_jiankang_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_jiankang_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_jiankang_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_jiankang_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'JK36', ch: 'health', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeJiankangModule.getJiankangNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      this.jiankangNewsList = arr;
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.jiankangSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.jiankangSwiperList = result[1].item;
      } else {
        this.jiankangSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.jiankangNewsList = this.jiankangNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
