<template>
  <div class="guoji-container" @scroll="monitorScrollEvent" ref="guoji-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="guoji-list" v-if="!pageLoading">
        <div class="swiper-container bg-white guojiSwiperList-container" style="height: 240px" v-if="guojiSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide guojiSwiperList-slide" v-for="(news, index) in guojiSwiperList" :key="index" style="width: 100%">
        ...
        <div class="guojiGridList" v-if="guojiGridList.length">
          <li v-for="(item, index) in guojiGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <van-notice-bar left-icon="volume-o" :scrollable="false" color="#1989fa" background="#ecf9ff" v-if="fastmessagescrollList.length">
          <van-swipe vertical class="yaowen-notice-swipe" :intlplay="3000" :show-indicators="false">
            <van-swipe-item v-for="(item, index) in fastmessagescrollList" :key="index" class="text-dot-1">{{ item.title }}</van-swipe-item>
          </van-swipe>
        </van-notice-bar>
        <div class="guojiNewsList" v-if="guojiNewsList.length">
          <li v-for="news in guojiNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeGuojiModule } from '@/store/modules/home_tab/guoji';
import { TabHomeChengshiModule } from '@/store/modules/home_tab/chengshi';
@Component({
  name: 'home_tab_item_guoji',
})
export default class extends Vue {
  ...
  public guojiNewsList: any[] = [];
  public guojiSwiperList: any[] = [];
  public guojiGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.guojiNewsList.length ? `已为您推荐 ${this.guojiNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['guoji-container'].scrollHeight;
    const scrollTop = this.$refs['guoji-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_guoji_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_guoji_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_guoji_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_guoji_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_guoji_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_guoji_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'GJPD', ch: 'intl', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeGuojiModule.getGuojiNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      this.guojiNewsList = arr;
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.guojiSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.guojiSwiperList = result[1].item;
      } else {
        this.guojiSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.guojiNewsList = this.guojiNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
