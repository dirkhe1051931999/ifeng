<template>
  <div class="jiaoyu-container" @scroll="monitorScrollEvent" ref="jiaoyu-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="jiaoyu-list" v-if="!pageLoading">
        <div class="swiper-container bg-white jiaoyuSwiperList-container" style="height: 240px" v-if="jiaoyuSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide jiaoyuSwiperList-slide" v-for="(news, index) in jiaoyuSwiperList" :key="index" style="width: 100%">
        ...
        <div class="jiaoyuGridList" v-if="jiaoyuGridList.length">
          <li v-for="(item, index) in jiaoyuGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <div class="jiaoyuNewsList" v-if="jiaoyuNewsList.length">
          <li v-for="news in jiaoyuNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeJiaoyuModule } from '@/store/modules/home_tab/jiaoyu';
@Component({
  name: 'home_tab_item_jiaoyu',
})
export default class extends Vue {
  ...
  public jiaoyuNewsList: any[] = [];
  public jiaoyuSwiperList: any[] = [];
  public jiaoyuGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.jiaoyuNewsList.length ? `已为您推荐 ${this.jiaoyuNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['jiaoyu-container'].scrollHeight;
    const scrollTop = this.$refs['jiaoyu-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_jiaoyu_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_jiaoyu_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_jiaoyu_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_jiaoyu_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_jiaoyu_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_jiaoyu_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'JY90', ch: 'educate', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeJiaoyuModule.getJiaoyuNewsList({ params, formData });
      const arr = [];
      ...
      this.jiaoyuNewsList = arr;
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.jiaoyuSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.jiaoyuSwiperList = result[1].item;
      } else {
        this.jiaoyuSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.jiaoyuNewsList = this.jiaoyuNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
