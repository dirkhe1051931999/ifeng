<template>
  <div class="taiwan-container" @scroll="monitorScrollEvent" ref="taiwan-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="taiwan-list" v-if="!pageLoading">
        <div class="swiper-container bg-white taiwanSwiperList-container" style="height: 240px" v-if="taiwanSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide taiwanSwiperList-slide" v-for="(news, index) in taiwanSwiperList" :key="index" style="width: 100%">
        ...
        <div class="taiwanGridList" v-if="taiwanGridList.length">
          <li v-for="(item, index) in taiwanGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <van-notice-bar left-icon="volume-o" :scrollable="false" color="#1989fa" background="#ecf9ff" v-if="fastmessagescrollList.length">
          <van-swipe vertical class="yaowen-notice-swipe" :twplay="3000" :show-indicators="false">
            <van-swipe-item v-for="(item, index) in fastmessagescrollList" :key="index" class="text-dot-1">{{ item.title }}</van-swipe-item>
          </van-swipe>
        </van-notice-bar>
        <div class="taiwanNewsList" v-if="taiwanNewsList.length">
          <li v-for="news in taiwanNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeTaiwanModule } from '@/store/modules/home_tab/taiwan';
import { TabHomeChengshiModule } from '@/store/modules/home_tab/chengshi';
@Component({
  name: 'home_tab_item_taiwan',
})
export default class extends Vue {
  ...
  public taiwanNewsList: any[] = [];
  public taiwanSwiperList: any[] = [];
  public taiwanGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.taiwanNewsList.length ? `已为您推荐 ${this.taiwanNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['taiwan-container'].scrollHeight;
    const scrollTop = this.$refs['taiwan-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_taiwan_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_taiwan_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_taiwan_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_taiwan_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_taiwan_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_taiwan_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'TW73', ch: 'tw', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeTaiwanModule.getTaiwanNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      this.taiwanNewsList = arr;
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.taiwanSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.taiwanSwiperList = result[1].item;
      } else {
        this.taiwanSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.taiwanNewsList = this.taiwanNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
