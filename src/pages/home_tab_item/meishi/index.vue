<template>
  <div class="meishi-container" @scroll="monitorScrollEvent" ref="meishi-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="meishi-list" v-if="!pageLoading">
        <div class="swiper-container bg-white meishiSwiperList-container" style="height: 240px" v-if="meishiSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide meishiSwiperList-slide" v-for="(news, index) in meishiSwiperList" :key="index" style="width: 100%">
        ...
        <div class="meishiGridList" v-if="meishiGridList.length">
          <li v-for="(item, index) in meishiGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <div class="meishiNewsList" v-if="meishiNewsList.length">
          <li v-for="news in meishiNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeMeishiModule } from '@/store/modules/home_tab/meishi';
@Component({
  name: 'home_tab_item_meishi',
})
export default class extends Vue {
  ...
  public meishiNewsList: any[] = [];
  public meishiSwiperList: any[] = [];
  public meishiGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.meishiNewsList.length ? `已为您推荐 ${this.meishiNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['meishi-container'].scrollHeight;
    const scrollTop = this.$refs['meishi-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_meishi_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_meishi_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_meishi_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_meishi_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_meishi_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_meishi_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'DELIC', ch: 'delic', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeMeishiModule.getMeishiNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      this.meishiNewsList = arr;
      for (let item of result[3].item) {
        if (item.type !== 'gregnewslist') {
          arr2.push(item);
        }
      }
      this.meishiGridList = arr2;
      if (result.length === 4) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.meishiSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.meishiSwiperList = result[2].item;
      } else {
        this.meishiSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.meishiNewsList = this.meishiNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
