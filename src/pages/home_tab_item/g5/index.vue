<template>
  <div class="g5-container" @scroll="monitorScrollEvent" ref="g5-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="g5-list" v-if="!pageLoading">
        <div class="swiper-container bg-white g5SwiperList-container" style="height: 240px" v-if="g5SwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide g5SwiperList-slide" v-for="(news, index) in g5SwiperList" :key="index" style="width: 100%">
        ...
        <div class="g5GridList" v-if="g5GridList.length">
          <li v-for="(item, index) in g5GridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <div class="g5NewsList" v-if="g5NewsList.length">
          <li v-for="news in g5NewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeG5Module } from '@/store/modules/home_tab/g5';
@Component({
  name: 'home_tab_item_g5',
})
export default class extends Vue {
  ...
  public g5NewsList: any[] = [];
  public g5SwiperList: any[] = [];
  public g5GridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.g5NewsList.length ? `已为您推荐 ${this.g5NewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['g5-container'].scrollHeight;
    const scrollTop = this.$refs['g5-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_g5_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_g5_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_g5_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_g5_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_g5_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_g5_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'KJ5G', ch: '5g', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeG5Module.getG5NewsList({ params, formData });
      const arr = [];
      ...
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.g5SwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.g5SwiperList = result[1].item;
      } else {
        this.g5SwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.g5NewsList = this.g5NewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
