<template>
  <div class="lishi-container" @scroll="monitorScrollEvent" ref="lishi-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="lishi-list" v-if="!pageLoading">
        <div class="swiper-container bg-white lishiSwiperList-container" style="height: 240px" v-if="lishiSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide lishiSwiperList-slide" v-for="(news, index) in lishiSwiperList" :key="index" style="width: 100%">
        ...
        <div class="lishiGridList" v-if="lishiGridList.length">
          <li v-for="(item, index) in lishiGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <div class="lishiNewsList" v-if="lishiNewsList.length">
          <li v-for="news in lishiNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeLishiModule } from '@/store/modules/home_tab/lishi';

@Component({
  name: 'home_tab_item_lishi',
})
export default class extends Vue {
  ...
  public lishiNewsList: any[] = [];
  public lishiSwiperList: any[] = [];
  public lishiGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.lishiNewsList.length ? `已为您推荐 ${this.lishiNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }

  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['lishi-container'].scrollHeight;
    const scrollTop = this.$refs['lishi-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_lishi_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_lishi_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_lishi_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_lishi_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_lishi_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_lishi_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'LS153', ch: 'history', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeLishiModule.getLishiNewsList({ params, formData });
      const arr = [];
      ...
      this.lishiNewsList = arr;
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.lishiSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.lishiSwiperList = result[1].item;
      } else {
        this.lishiSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.lishiNewsList = this.lishiNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
