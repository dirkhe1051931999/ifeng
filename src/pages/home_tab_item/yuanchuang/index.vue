<template>
  <div class="yuanchuang-container" @scroll="monitorScrollEvent" ref="yuanchuang-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="yuanchuang-list" v-if="!pageLoading">
        <div class="swiper-container bg-white yuanchuangSwiperList-container" style="height: 240px" v-if="yuanchuangSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide yuanchuangSwiperList-slide" v-for="(news, index) in yuanchuangSwiperList" :key="index" style="width: 100%">
        ...
        <div class="yuanchuangGridList" v-if="yuanchuangGridList.length">
          <li v-for="(item, index) in yuanchuangGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <van-notice-bar left-icon="volume-o" :scrollable="false" color="#1989fa" background="#ecf9ff" v-if="fastmessagescrollList.length">
          <van-swipe vertical class="yaowen-notice-swipe" :solecolumnplay="3000" :show-indicators="false">
            <van-swipe-item v-for="(item, index) in fastmessagescrollList" :key="index" class="text-dot-1">{{ item.title }}</van-swipe-item>
          </van-swipe>
        </van-notice-bar>
        <div class="yuanchuangNewsList" v-if="yuanchuangNewsList.length">
          <li v-for="news in yuanchuangNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeYuanchuangModule } from '@/store/modules/home_tab/yuanchuang';
import { TabHomeChengshiModule } from '@/store/modules/home_tab/chengshi';
@Component({
  name: 'home_tab_item_yuanchuang',
})
export default class extends Vue {
  ...
  public yuanchuangNewsList: any[] = [];
  public yuanchuangSwiperList: any[] = [];
  public yuanchuangGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.yuanchuangNewsList.length ? `已为您推荐 ${this.yuanchuangNewsList.length} 条新内容` : '已更新到最新';
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_yuanchuang_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_yuanchuang_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_yuanchuang_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_yuanchuang_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_yuanchuang_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_yuanchuang_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'SOLE', ch: 'solecolumn', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeYuanchuangModule.getYuanchuangNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      console.log(arr);
      this.yuanchuangNewsList = arr;
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.yuanchuangSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.yuanchuangSwiperList = result[1].item;
      } else {
        this.yuanchuangSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.yuanchuangNewsList = this.yuanchuangNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
