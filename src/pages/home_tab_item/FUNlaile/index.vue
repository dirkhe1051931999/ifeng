<template>
  <div class="FUNlaile-container" @scroll="monitorScrollEvent" ref="FUNlaile-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="FUNlaile-list" v-if="!pageLoading">
        <div class="swiper-container bg-white FUNlaileSwiperList-container" style="height: 240px" v-if="FUNlaileSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide FUNlaileSwiperList-slide" v-for="(news, index) in FUNlaileSwiperList" :key="index" style="width: 100%">
        ...
        <div class="FUNlaileGridList" v-if="FUNlaileGridList.length">
          <li v-for="(item, index) in FUNlaileGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <div class="FUNlaileNewsList" v-if="FUNlaileNewsList.length">
          <li v-for="news in FUNlaileNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeFUNlaileModule } from '@/store/modules/home_tab/FUNlaile';
@Component({
  name: 'home_tab_item_FUNlaile',
})
export default class extends Vue {
  ...
  public FUNlaileNewsList: any[] = [];
  public FUNlaileSwiperList: any[] = [];
  public FUNlaileGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.FUNlaileNewsList.length ? `已为您推荐 ${this.FUNlaileNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['FUNlaile-container'].scrollHeight;
    const scrollTop = this.$refs['FUNlaile-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_FUNlaile_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_FUNlaile_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_FUNlaile_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_FUNlaile_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_FUNlaile_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_FUNlaile_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'DZPD', ch: 'cmd', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeFUNlaileModule.getFUNlaileNewsList({ params, formData });
      const arr = [];
      ...
      this.FUNlaileNewsList = arr;
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.FUNlaileSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.FUNlaileSwiperList = result[1].item;
      } else {
        this.FUNlaileSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.FUNlaileNewsList = this.FUNlaileNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
