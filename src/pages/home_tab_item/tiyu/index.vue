<template>
  <div class="tiyu-container" @scroll="monitorScrollEvent" ref="tiyu-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="tiyu-list" v-if="!pageLoading">
        <div class="swiper-container bg-white tiyuSwiperList-container" style="height: 240px" v-if="tiyuSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide tiyuSwiperList-slide" v-for="(news, index) in tiyuSwiperList" :key="index" style="width: 100%">
        ...
        <div class="tiyuGridList" v-if="tiyuGridList.length">
          <li v-for="(item, index) in tiyuGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <div class="tiyuNewsList" v-if="tiyuNewsList.length">
          <li v-for="news in tiyuNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeTiyuModule } from '@/store/modules/home_tab/tiyu';
@Component({
  name: 'home_tab_item_tiyu',
})
export default class extends Vue {
  ...
  public tiyuNewsList: any[] = [];
  public tiyuSwiperList: any[] = [];
  public tiyuGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.tiyuNewsList.length ? `已为您推荐 ${this.tiyuNewsList.length} 条新内容` : '已更新到最新';
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'TY43', ch: 'sports', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeTiyuModule.getTiyuNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      this.tiyuNewsList = arr;
      for (let item of result[3].item) {
        if (item.type !== 'gregnewslist') {
          arr2.push(item);
        }
      }
      this.tiyuGridList = arr2;
      if (result.length === 4) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.tiyuSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.tiyuSwiperList = result[2].item;
      } else {
        this.tiyuSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.tiyuNewsList = this.tiyuNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
