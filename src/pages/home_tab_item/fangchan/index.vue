<template>
  <div class="fangchan-container" @scroll="monitorScrollEvent" ref="fangchan-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="fangchan-list" v-if="!pageLoading">
        <div class="swiper-container bg-white fangchanSwiperList-container" style="height: 240px" v-if="fangchanSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide fangchanSwiperList-slide" v-for="(news, index) in fangchanSwiperList" :key="index" style="width: 100%">
        ...
        <div class="fangchanGridList" v-if="fangchanGridList.length">
          <li v-for="(item, index) in fangchanGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <van-notice-bar left-icon="volume-o" :scrollable="false" color="#1989fa" background="#ecf9ff" v-if="fastmessagescrollList.length">
          <van-swipe vertical class="yaowen-notice-swipe" :houseplay="3000" :show-indicators="false">
            <van-swipe-item v-for="(item, index) in fastmessagescrollList" :key="index" class="text-dot-1">{{ item.title }}</van-swipe-item>
          </van-swipe>
        </van-notice-bar>
        <div class="fangchanNewsList" v-if="fangchanNewsList.length">
          <li v-for="news in fangchanNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeFangchanModule } from '@/store/modules/home_tab/fangchan';
import { TabHomeChengshiModule } from '@/store/modules/home_tab/chengshi';
@Component({
  name: 'home_tab_item_fangchan',
})
export default class extends Vue {
  ...
  public fangchanNewsList: any[] = [];
  public fangchanSwiperList: any[] = [];
  public fangchanGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.fangchanNewsList.length ? `已为您推荐 ${this.fangchanNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['fangchan-container'].scrollHeight;
    const scrollTop = this.$refs['fangchan-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_fangchan_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_fangchan_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_fangchan_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_fangchan_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_fangchan_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_fangchan_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'FC81', ch: 'house', action: 'default', pullTotal: '1', dailyOpenNum: '1', choicename: this.currentRegion };
      }
      const result = await TabHomeFangchanModule.getFangchanNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      this.fangchanNewsList = arr;
      for (let item of result[2].item) {
        if (item.type !== 'gregnewslist') {
          arr2.push(item);
        }
      }
      this.fangchanGridList = arr2;
      if (result.length === 3) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.fangchanSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.fangchanSwiperList = result[1].item;
      } else {
        this.fangchanSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.fangchanNewsList = this.fangchanNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
