<template>
  <div class="tuijian-container" @scroll="monitorScrollEvent" ref="tuijian-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="tuijian-list" v-if="!pageLoading">
        <div class="swiper-container bg-white tuijianSwiperList-container" style="height: 240px" v-if="tuijianSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide tuijianSwiperList-slide" v-for="(news, index) in tuijianSwiperList" :key="index" style="width: 100%">
        ...
        <div class="tuijianGridList" v-if="tuijianGridList.length">
          <li v-for="(item, index) in tuijianGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <div class="tuijianNewsList" v-if="tuijianNewsList.length">
          <li v-for="news in tuijianNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeTuijianModule } from '@/store/modules/home_tab/tuijian';
@Component({
  name: 'home_tab_item_tuijian',
})
export default class extends Vue {
  ...
  public tuijianNewsList: any[] = [];
  public tuijianSwiperList: any[] = [];
  public tuijianGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.tuijianNewsList.length ? `已为您推荐 ${this.tuijianNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['tuijian-container'].scrollHeight;
    const scrollTop = this.$refs['tuijian-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_tuijian_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_tuijian_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_tuijian_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_tuijian_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_tuijian_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_tuijian_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'RECOM', ch: 'rcmd', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeTuijianModule.getTuijianNewsList({ params, formData });
      const arr = [];
      ...
      this.tuijianNewsList = arr;
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.tuijianSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.tuijianSwiperList = result[1].item;
      } else {
        this.tuijianSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.tuijianNewsList = this.tuijianNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
