<template>
  <div class="dianshangyanjiuyuan-container" @scroll="monitorScrollEvent" ref="dianshangyanjiuyuan-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="dianshangyanjiuyuan-list" v-if="!pageLoading">
        <div class="swiper-container bg-white dianshangyanjiuyuanSwiperList-container" style="height: 240px" v-if="dianshangyanjiuyuanSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide dianshangyanjiuyuanSwiperList-slide" v-for="(news, index) in dianshangyanjiuyuanSwiperList" :key="index" style="width: 100%">
        ...
        <div class="dianshangyanjiuyuanGridList" v-if="dianshangyanjiuyuanGridList.length">
          <li v-for="(item, index) in dianshangyanjiuyuanGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <van-notice-bar left-icon="volume-o" :scrollable="false" color="#1989fa" background="#ecf9ff" v-if="fastmessagescrollList.length">
          <van-swipe vertical class="yaowen-notice-swipe" :dsyjyplay="3000" :show-indicators="false">
            <van-swipe-item v-for="(item, index) in fastmessagescrollList" :key="index" class="text-dot-1">{{ item.title }}</van-swipe-item>
          </van-swipe>
        </van-notice-bar>
        <div class="dianshangyanjiuyuanNewsList" v-if="dianshangyanjiuyuanNewsList.length">
          <li v-for="news in dianshangyanjiuyuanNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeDianshangyanjiuyuanModule } from '@/store/modules/home_tab/dianshangyanjiuyuan';
import { TabHomeChengshiModule } from '@/store/modules/home_tab/chengshi';
@Component({
  name: 'home_tab_item_dianshangyanjiuyuan',
})
export default class extends Vue {
  $refs: any;
  get currentTabId() {
    return 'dianshangyanjiuyuan';
  }
  get activeTabIndex() {
  ...
  async watchActiveTabIndex(newVal: number, oldVal: number) {
    if (newVal === this.currentIndex) {
      if (!this.currentPageIsLoaded) {
        ...
        this.dianshangyanjiuyuanNewsList = [];
  ...
  public dianshangyanjiuyuanNewsList: any[] = [];
  public dianshangyanjiuyuanSwiperList: any[] = [];
  public dianshangyanjiuyuanGridList: any[] = [];
  public fastmessagescrollList = [];
  // 下拉刷新，上拉加载的数据
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.dianshangyanjiuyuanNewsList.length ? `已为您推荐 ${this.dianshangyanjiuyuanNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['dianshangyanjiuyuan-container'].scrollHeight;
    const scrollTop = this.$refs['dianshangyanjiuyuan-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_dianshangyanjiuyuan_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_dianshangyanjiuyuan_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_dianshangyanjiuyuan_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_dianshangyanjiuyuan_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_dianshangyanjiuyuan_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_dianshangyanjiuyuan_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'DSYJY', ch: 'dsyjy', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeDianshangyanjiuyuanModule.getDianshangyanjiuyuanNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      if (result.length === 2) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.dianshangyanjiuyuanSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.dianshangyanjiuyuanSwiperList = result[1].item;
      } else {
        this.dianshangyanjiuyuanSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.dianshangyanjiuyuanNewsList = this.dianshangyanjiuyuanNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
