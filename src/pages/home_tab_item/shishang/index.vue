<template>
  <div class="shishang-container" @scroll="monitorScrollEvent" ref="shishang-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="shishang-list" v-if="!pageLoading">
        <div class="swiper-container bg-white shishangSwiperList-container" style="height: 240px" v-if="shishangSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide shishangSwiperList-slide" v-for="(news, index) in shishangSwiperList" :key="index" style="width: 100%">
        ...
        <div class="shishangGridList" v-if="shishangGridList.length">
          <li v-for="(item, index) in shishangGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <van-notice-bar left-icon="volume-o" :scrollable="false" color="#1989fa" background="#ecf9ff" v-if="fastmessagescrollList.length">
          <van-swipe vertical class="yaowen-notice-swipe" :fashionplay="3000" :show-indicators="false">
            <van-swipe-item v-for="(item, index) in fastmessagescrollList" :key="index" class="text-dot-1">{{ item.title }}</van-swipe-item>
          </van-swipe>
        </van-notice-bar>
        <div class="shishangNewsList" v-if="shishangNewsList.length">
          <li v-for="news in shishangNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeShishangModule } from '@/store/modules/home_tab/shishang';
import { TabHomeChengshiModule } from '@/store/modules/home_tab/chengshi';
@Component({
  name: 'home_tab_item_shishang',
})
export default class extends Vue {
  ...
  public shishangNewsList: any[] = [];
  public shishangSwiperList: any[] = [];
  public shishangGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.shishangNewsList.length ? `已为您推荐 ${this.shishangNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['shishang-container'].scrollHeight;
    const scrollTop = this.$refs['shishang-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_shishang_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_shishang_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_shishang_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_shishang_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_shishang_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_shishang_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'SS78', ch: 'fashion', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeShishangModule.getShishangNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      this.shishangGridList = arr2;
      if (!arr.length) {
        this.load_more_no_data = '没有更多数据了';
      }
      this.shishangNewsList = arr;
      if (result.length === 3) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.shishangSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.shishangSwiperList = result[1].item;
      } else {
        this.shishangSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.shishangNewsList = this.shishangNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
