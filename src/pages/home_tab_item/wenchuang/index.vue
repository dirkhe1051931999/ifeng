<template>
  <div class="wenchuang-container" @scroll="monitorScrollEvent" ref="wenchuang-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="wenchuang-list" v-if="!pageLoading">
        <div class="swiper-container bg-white wenchuangSwiperList-container" style="height: 240px" v-if="wenchuangSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide wenchuangSwiperList-slide" v-for="(news, index) in wenchuangSwiperList" :key="index" style="width: 100%">
        ...
        <div class="wenchuangGridList" v-if="wenchuangGridList.length">
          <li v-for="(item, index) in wenchuangGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <div class="wenchuangNewsList" v-if="wenchuangNewsList.length">
          <li v-for="news in wenchuangNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeWenchuangModule } from '@/store/modules/home_tab/wenchuang';
@Component({
  name: 'home_tab_item_wenchuang',
})
export default class extends Vue {
  ...
  public wenchuangNewsList: any[] = [];
  public wenchuangSwiperList: any[] = [];
  public wenchuangGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.wenchuangNewsList.length ? `已为您推荐 ${this.wenchuangNewsList.length} 条新内容` : '已更新到最新';
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'WENCHUANG', ch: 'cultural', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeWenchuangModule.getWenchuangNewsList({ params, formData });
      const arr = [];
      ...
      this.wenchuangNewsList = arr;
      this.wenchuangGridList = result[2].item;
      if (result.length === 3) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.wenchuangSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.wenchuangSwiperList = result[1].item;
      } else {
        this.wenchuangSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.wenchuangNewsList = this.wenchuangNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
