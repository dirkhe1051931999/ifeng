<template>
  <div class="lvyou-container" @scroll="monitorScrollEvent" ref="lvyou-container">
    <van-pull-refresh v-model="isDownRefresh" :success-text="refreshSuccessText" @refresh="onRefresh" :success-duration="1000">
      ...
      <ul class="lvyou-list" v-if="!pageLoading">
        <div class="swiper-container bg-white lvyouSwiperList-container" style="height: 240px" v-if="lvyouSwiperList.length">
          <div class="swiper-wrapper">
            <div class="swiper-slide lvyouSwiperList-slide" v-for="(news, index) in lvyouSwiperList" :key="index" style="width: 100%">
        ...
        <div class="lvyouGridList" v-if="lvyouGridList.length">
          <li v-for="(item, index) in lvyouGridList" :key="index" @click="handlerClickGridItem(item, index)">
        ...
        <van-notice-bar left-icon="volume-o" :scrollable="false" color="#1989fa" background="#ecf9ff" v-if="fastmessagescrollList.length">
          <van-swipe vertical class="yaowen-notice-swipe" :travelplay="3000" :show-indicators="false">
            <van-swipe-item v-for="(item, index) in fastmessagescrollList" :key="index" class="text-dot-1">{{ item.title }}</van-swipe-item>
          </van-swipe>
        </van-notice-bar>
        <div class="lvyouNewsList" v-if="lvyouNewsList.length">
          <li v-for="news in lvyouNewsList" :key="news.id + Math.random().toString()">
            <!-- topic2 -->
...
import { TabHomeLvyouModule } from '@/store/modules/home_tab/lvyou';
import { TabHomeChengshiModule } from '@/store/modules/home_tab/chengshi';
@Component({
  name: 'home_tab_item_lvyou',
})
export default class extends Vue {
  ...
  public lvyouNewsList: any[] = [];
  public lvyouSwiperList: any[] = [];
  public lvyouGridList: any[] = [];
  ...
  async onRefresh() {
    ...
    await this._downCallback();
    this.refreshSuccessText = this.lvyouNewsList.length ? `已为您推荐 ${this.lvyouNewsList.length} 条新内容` : '已更新到最新';
    this.isDownRefresh = false;
  }
  async monitorScrollEvent(e: any) {
    const scrollHeight = this.$refs['lvyou-container'].scrollHeight;
    const scrollTop = this.$refs['lvyou-container'].scrollTop;
  ...
  public handlerClickGridItem(item: any, index: number) {
    ...
    // switch (item.title) {
    //   case '科技热榜':
    //     this.$router.push('/tab_lvyou_child/rebang');
    //     break;
    //   case '新视界':
    //     this.$router.push('/tab_lvyou_child/xinshijie');
    //     break;
    //   case '凰家评测':
    //     this.$router.push('/tab_lvyou_child/pingce');
    //     break;
    //   case '坏消息':
    //     this.$router.push('/tab_lvyou_child/huaixiaoxi');
    //     break;
    //   case '原创':
    //     this.$router.push('/tab_lvyou_child/yuanchuang');
    //     break;
    //   case '深度':
    //     this.$router.push('/tab_lvyou_child/shendu');
    //     break;
    //   default:
    //     break;
    // }
  ...
  public async _downCallback() {
    ...
    try {
      if (this.firstLoadData) {
        params = { id: 'LY67', ch: 'travel', action: 'default', pullTotal: '1', dailyOpenNum: '1' };
      }
      const result = await TabHomeLvyouModule.getLvyouNewsList({ params, formData });
      const arr = [];
      const arr2 = [];
      ...
      this.lvyouGridList = arr2;
      if (!arr.length) {
        this.load_more_no_data = '没有更多数据了';
      }
      this.lvyouNewsList = arr;
      if (result.length === 3) {
        this.$nextTick(() => {
          if (this.firstLoadData) {
            setTimeout(() => {
              new window['Swiper']('.lvyouSwiperList-container', {
                loop: true,
              });
            }, 500);
          }
        });
        this.lvyouSwiperList = result[1].item;
      } else {
        this.lvyouSwiperList = [];
      }
  ...
  public async _upCallback() {
    ...
    try {
      ...
      this.lvyouNewsList = this.lvyouNewsList.concat(arr);
      return Promise.resolve(true);
    } catch (error) {
